...
export default function VintageChart({ rows }: Props) {
    ...
    >
      ...
      <ResponsiveContainer width="100%" height={360}>
        <ScatterChart margin={{ top: 10, right: 20, bottom: 10, left: 0 }}>
          ...
          />
          <ZAxis dataKey="fundSizeUSDm" range={[40, 400]} />
          <Tooltip
            content={({ payload }) => {
              ...
              return (
                <div className="bg-white/95 backdrop-blur-sm border border-slate-200 rounded-xl px-3 py-2 shadow-lg text-xs">
                  ...
                  <p className="text-slate-500">{d.firm} · {d.vintage} · {d.strategyType}</p>
                  <p className="text-slate-600 mt-1">
                    Net TVPI: <span className="font-medium">{fmtX(d.netTVPI)}</span> · Size: {fmtUSDm(d.fundSizeUSDm)}
                  </p>
                </div>
              );
            }}
          ...
          {firms.map((firm) => (
            <Scatter
              key={firm}
              name={firm}
              data={rows.filter((r) => r.firm === firm && r.netTVPI != null)}
      ...
      <div className="chart-insight">
        <strong>Insight:</strong> Earlier vintages (pre-2015) show dramatically higher multiples. As fund sizes grow, TVPI compression is evident across all firms.
      </div>
    </motion.div>
  );
}
